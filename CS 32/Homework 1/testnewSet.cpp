...
int main()
{
	...
	assert(t1.size() == 5);//testing the size() function
	assert(t2.size() == 3);//testing the size() function
	...
	Set t3;
	assert(t3.empty());//testing if the the newly created Set is empty
	for (int i = 0; i < 100; i++)
	{
	...
	Set t4;
	assert(t4.empty());//testing if the the newly created Set is empty
	...
	ItemType k;
	Set t5;
	t5.insert(45);//checking insert function
	t5.get(0, k);
