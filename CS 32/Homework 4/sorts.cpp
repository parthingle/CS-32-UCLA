...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
	...
	/*for (vector<Sensor>::iterator p = s.begin(); p != s.end(); p++)
	{
		vector<Sensor>::iterator q = p;

		while ((q != s.begin() && (q->avg < (q - 1)->avg)))
		{
			double temp = q->avg;
			q->avg = (q - 1)->avg;
			(q - 1)->avg = temp;
	...
	else
	{
		for (unsigned int k = 2; k <= s.size(); k++)
		{
			...
			if (comp(s[k - 1], s[k - 2]))
			{
				while (comp(s[k - index + 1], s[k - index]) && k - index >= 0)
				{
					Sensor q = s[k - index + 1];
					s[k - index + 1] = s[k - index];
					s[k - index] = q;
					index++;
				}
			}
...
int main()
{
	...
	// destruction time.
	{
		...
		vector<Sensor*> auxSensorPtr;
		auxSensorPtr.resize(auxSensors.size());
		//int j;
		for (unsigned int k = 0; k < auxSensors.size(); k++)
		{	
			auxSensorPtr[k] = &auxSensors[k];
		}

		// TODO:  Sort the vector of pointers using the STL sort algorithm
		//        with compareSensorPtr as the ordering relationship.

		vector<Sensor*>::iterator it =auxSensorPtr.begin();
		vector<Sensor*>::iterator it1=auxSensorPtr.end();

		sort(it, it1, compareSensorPtr);
		...
		for (unsigned int k = 0; k < auxSensorPtr.size(); k++)
		{
			sensors[k] = *(auxSensorPtr[k]);
		}
